...
Using IMU sensors for human activity recognition (HAR) has been widely studied in the literature and differs from IMUs' original purpose of motion tracking.
Take step counting as an example, generally we don't calculate the horizontal position change of the sensor, which relies on the tracking algorithm like Kalman filter. Instead, we do not care about the exact position change, but detect the periodic pattern in the acceleration caused by the footsteps. 
The similar principle can be applied to more complicated activities, such as running, jumping, and cycling.
To do so, deep learning-based methods have been widely adopted in recent years, generally consisting of two stages: feature extraction and activity classification. Most deep learning models remain simple, mainly due to the lack of large-scale datasets and the inherent limitations of IMU signals.
