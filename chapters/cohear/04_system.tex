...
To enable the pairing of networks, we consider two features as discussed in Sec. \ref{sec:cohear_related}: verbal and non-verbal. Since the verbal feature (turn-taking) needs to wait for enough time before being effective, we consider the non-verbal feature as the main factor to determine the conversation.
...
As for the sound localization, we extract the cross-channel features as Sec. \ref{sec:cohear_background} and utilize a convolutional recurrent neural network (CRNN) to process them, which consists of multiple convolutional layers, a GRU layer, and the final linear layer to predict the output. 
