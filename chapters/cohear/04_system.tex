...
\label{sec:cohear_system}


\subsection{Design Overview}
\label{sec:cohear_overview}
\begin{figure}[h]
    \centering
...
CoHear establishes connections based on conversational features, as illustrated in Fig. \ref{fig:system overview}. To achieve this, we define a ``conversation network'' as the foundation of the system, where individuals participating in the same conversation receive conversation enhancement.
...
First, the \textbf{node discovery} module acts as the front end of our system, where nearby nodes (i.e., speakers) are identified using deep learning-based sound source localization. We extract both the locations and embeddings of each speaker to serve as input for the next stage. Note that this process applies to all speakers wearing earphones; further details are provided in Sec. \ref{sec:cohear_acoustic}.
Second, \textbf{geometric calibration} is performed in a centralized manner by aggregating the outputs of the node discovery module from each speaker, as discussed in Sec. \ref{sec:cohear_geometric}. Although each node can only observe others within its local coordinate system, we iteratively optimize a shared global geometry. The more observations we obtain from the speakers, the more stable the optimization becomes. After completing the initial optimization with a cold start, we can track the speakers in real time.
Once the conversation network is established, the final component, \textbf{conversation enhancement}, performs conversation-level speech enhancement based on the identified participants (described in Sec. \ref{sec:cohear_conversation}). Specifically, both the speaker embedding (time-invariant) and the relay signal (time-variant) are used as cues for the conditional speech enhancement model. We introduce an audio control module to balance the contributions of these two approaches.
...
To enable the pairing of networks, we consider two features verbal and non-verbal. Since the verbal feature (turn-taking) needs to wait for enough time before being effective, we consider the non-verbal feature as the main factor to determine the conversation.
...
\label{sec:cohear_acoustic}
Conversation can be modeled through interactions between speakers. In CoHear, each user is treated as a node, and speaker localization is reformulated as dynamic node discovery in a mobile acoustic network, as shown in Fig.~\ref{fig:acoustic}. The system estimates both the location and identity of nearby nodes. Location is represented in the receiver's local coordinate system, and identity is represented by a voiceprint embedding.
...
For sound localization, we extract the cross-channel features described in Sec.~\ref{sec:cohear_background} and process them with a convolutional recurrent neural network (CRNN). The CRNN consists of multiple convolutional layers, a GRU layer, and a final linear layer.
...
After localizing all sound sources, the next step is to associate each sound location with a speaker identity. Existing tools such as Pyannote~\cite{bredin2023pyannote} can estimate speaker identities under overlapping speech and multiple-speaker conditions.
However, associating locations with identities is difficult because they are estimated as independent attributes. In particular, source locations can be permuted without changing the localization result.
...
We use a joint model for beamforming and embedding extraction. Specifically, we keep the beamformer (e.g., delay-and-sum beamformer) unchanged, reshape the output from \(B \times C \times T \times F\) to \(B \times T \times (F \times C)\), and use a linear projector with layer normalization to predict the d-vector. The final d-vector is averaged over time. Since the output is not audio, the common neural beamforming loss (i.e., scale-invariant signal-to-noise ratio) is not applicable; we instead use cosine similarity loss:
...
\label{sec:cohear_geometric}
As outlined in our system overview (Sec. \ref{sec:cohear_overview}), our ultimate goal is to estimate the positions of all users (nodes) in a conversation network. The previous node discovery step identifies nearby speakers, providing important positional observations. However, these outputs only indicate where people are speaking, not the structure of the conversation network itself. To achieve this, we propose aggregating observations from multiple participants to estimate the network's underlying geometry.
This task can be formulated as a multi-sensor localization problem, known as geometric calibration. Given that we use acoustic signals for localization, our approach falls within the category of WASN, as discussed in Section \ref{sec:cohear_background}.
...
\label{sec:cohear_sharing}
Geometric calibration relies on sources observed by multiple nodes, where each relative position constrains the node positions. The system must therefore align source observations across nodes.
This alignment is simple with one sound source but becomes difficult when sounds overlap or when some nodes cannot observe a source. Since the corresponding speaker embedding has been extracted for each source, sources with similar embeddings can be assigned the same identity.
...
A group of sources with more than 0.8 similarity is considered the same source. We do not assume that every source can be observed by all users.
In practice, every device uploads its observed embeddings from Sec. \ref{sec:cohear_acoustic} to a central server and performs the above matching. Only the source that is observed by at least two nodes are considered as valid ones in the following.

\paragraph{Distance estimation}
\label{sec:cohear_distance}
According to the output of Sec. \ref{sec:cohear_acoustic}, we can only obtain the direction of arrival, while the distance to the sound source is skipped.
...
Despite this design, distance estimation errors may still be significant. Therefore, we do not use the estimated distance as a final output. Instead, we use it as one input to subsequent geometric calibration.



\paragraph{Mobile WASN}
According to Sec. \ref{sec:cohear_background}, our network can be considered as mobile WASN, an extension of WASN where the acoustic node can move freely. Similarly, localization of sensor nodes can be achieved by geometric calibration, given sufficient observed sound sources. Compared to the conventional notation, mobile WASN has different assumptions as follows.
...
We perform mobile geometric calibration with networked earphones. The method follows the same principle as other geometric calibration approaches: it minimizes a cost function that measures how well converted observations align with the assumed geometry. Among existing algorithms, dataset matching is an efficient approach for geometric calibration. Our algorithm builds on \cite{gburrek2021geometry}; this chapter focuses on the extensions needed for mobile earphone networks.
...
Assuming the sound source localization method from Section \ref{sec:cohear_acoustic} provides a reliable DOA, we note that the distance and source orientation variables are still missing. To address this, we modify our model to estimate these two additional variables.
...
\label{sec:cohear_conversation}


\paragraph{Problem formulation}
For each group of nodes participating in the same conversation, the locations, identities (represented by speaker embeddings), and audio recordings are shared among all members. CoHear uses this information to extract the target conversation, as shown in Fig.~\ref{fig:voice_share}.

The design goal is to extract a target conversation from a speech mixture under specific conditions. Raw audio provides more fine-grained information than speaker embeddings, but it consumes substantially more bandwidth. Directly transmitting relay audio, as in a video conference, is impractical for two reasons. First, even after noise suppression, the transmitted audio may not match the target speech. Second, transmission introduces delay, packet loss, and compression artifacts. The extraction model must therefore use degraded relay audio effectively.
...
Transmission delay is another constraint. Delayed relay audio becomes less useful even when its audio quality is high because temporal correlation between the time-varying feature and the target speech weakens.
...
Specifically, we use GCC between the transmitted audio and the noisy speech, identifying the first peak as the delay.
We do not need to estimate the delay frequently because it is assumed to be relatively stable.
...
CoHear must also support multiple conditioning sources from multiple participants. A naive solution is to apply the speech filter multiple times, but this is computation-intensive. Instead, we use an attention mechanism that combines multiple conditions into the intermediate feature:
\(
 X = Attention([X_0, X_1, ...])
\)
