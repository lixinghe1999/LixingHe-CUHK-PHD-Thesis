...
In the last session, we asked the same participants to evaluate CoHear's usability in real-world settings. Since CoHear is built on existing headphones and does not change the form factor, we omit questions about wearing experience.
\begin{itemize}
    \item Do you feel that you cannot hear others clearly in a noisy environment?
...
CoHear uses relay audio as a condition instead of relying only on speaker embeddings. This choice makes relay audio quality critical to system performance, which we assess across four aspects, as illustrated in Fig.~\ref{fig:relay_audio_quality}.
...
We assess the performance of the experience-based controller in Fig.~\ref{fig:controller}. The comparison includes two naive baselines: one that assigns equal bandwidth to each stream, and one that allocates maximum bandwidth sequentially until the available bandwidth is exhausted. During the experiments, we sample the number of nodes, input SNR, and available bandwidth randomly. The average SNR is shown in the figure. The controller improves SNR by approximately 1 dB without requiring additional bandwidth.
