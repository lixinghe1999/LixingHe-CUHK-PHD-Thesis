...
Moreover, CoHear supports time-invariant speaker embeddings that require only one-time transmission, significantly reducing data size to 8192 bits in total, which is practical on even constrained networks.
...
For conversation enhancement, we use scale-invariant signal-to-noise ratio (Si-SNR) as the main metric. The default input SNR is set to 0 dB, so the output SNR reflects improvement over the input.
...
For geometric calibration, we use \cite{gburrek2021geometry} as the baseline because it is designed for static WASN. We also compare against infrastructure-based acoustic localization \cite{gburrek2021geometry} and acoustic-SLAM \cite{evers2018acoustic}. When \cite{gburrek2021geometry} is used as infrastructure-based acoustic localization, the nodes are static and the sources are mobile users; the target is therefore to estimate sound-source positions rather than node positions. We use DSM for the static WASN baseline \cite{gburrek2021geometry}, iDSM for infrastructure-based acoustic localization \cite{gburrek2021geometry}, and aSLAM for acoustic-SLAM \cite{evers2018acoustic}.
...
We use LOTH for LookOnceToHear \cite{veluri2024look}, TC for \cite{chen2024target}, and CoS for \cite{jenrungrot2020cone}.
