...
\paragraph{EgoADL:}
EgoADL \citep{sun2024multimodal} is also a large-scale dataset collected in the real world with Android smartphones, including Wi-Fi, audio, and IMU data. We exclude Wi-Fi data for a fair comparison and perform similar clustering for the 221 action classes in the dataset.
...
We use the following evaluation metrics. For multi-label classification (i.e., scenario and spatial classification), we use multi-label F1 score: $2*TP/(2*TP+FP+FN)$. For single-label classification (i.e., activity recognition), we use accuracy.
...
To evaluate EgoLog from a user perspective, we conducted a study in which volunteers provided subjective ratings of the system outputs. The goal is to provide users with a daily log that summarizes their activities through two complementary outputs: scenarios and activities.
