...
\begin{figure*}
  ...
  \caption{Overview of the \textit{VibVoice} system. VibVoice augments acceleration data using a large public speech dataset and estimated Bone Conduction Functions, then trains a multimodal DNN for speech reconstruction.}
  \label{fig:vibvoice_overview}
\end{figure*}
Motivated by the findings in Section~\ref{sec:vibvoice_measurement}, bone-conducted vibration can complement microphone recording for speech enhancement under environmental noise.
However, the limited sampling rate of HMW accelerometers and the diverse frequency responses of bone-conducted vibration introduce challenges for multimodal speech enhancement.
...
We design \textit{VibVoice}, a speech enhancement system for HMWs that uses both microphone recordings and speech information from bone-conducted vibration.
Fig. \ref{fig:vibvoice_overview} summarizes the design of VibVoice, which contains three components: Bone Conduction Function estimation, data augmentation, and a speech reconstruction network.
...
This thresholding setting uses temporal information and suppresses noise more effectively than a constant threshold in the frequency or time domain.
...
We model the Bone Conduction Function using a Gaussian distribution in the frequency domain because the frequency response (Fig.~\ref{fig:vibvoice_bcf}) has substantial variance due to the complex structure of the skull \citep{chang2016development}.
...
We use these functions to generate acceleration signals from the large-scale LibriSpeech audio dataset~\citep{panayotov2015librispeech}.
...
We generate a pool of Bone Conduction Functions by interpolating the measured functions and sampling a Gaussian distribution in each frequency band. This preserves their low-pass characteristics and anatomical variation while reducing overfitting. The eight participants used to construct this pool are disjoint from the 15 participants used for training and evaluation. We use leave-one-user-out validation on the latter group, so the held-out user contributes no data to function estimation, augmentation, or model training.

Although VibVoice does not require target-user data for training, a small amount of target-domain data can further improve performance. Such data can be collected during normal HMW setup, such as voice-assistant personalization, without adding a separate calibration step. VibVoice can also use speech recorded in quiet environments. Because the collection does not require specific content, the system can accumulate user-specific data during long-term HMW use and improve personalization over time.
...
\begin{split}
    L_{\mathrm{STFT}}(y, \hat{y}) &= L_{sc}(y, \hat{y}) + L_{mag}(y, \hat{y}),
    \\
...
\begin{split}
    L(y, \hat{y}) &= L_{\mathrm{STFT}}(y, \hat{y}) + |y_{lowband} - \hat{y}_{lowband}|_{2} \times \lambda,
\end{split}
\end{align}
